...
def draw(var_dic, station='zhengzhou'):
    ...
    for model,color in zip(model_list,color_list):
        ...
        axes[0].plot(var_dic['temp'][model].values, var_dic['temp'][model].pressure, color=color, label=model)
        axes[1].plot(var_dic['theta_v'][model].values, var_dic['theta_v'][model].pressure, color=color, label=model)
        axes[2].plot(var_dic['rh'][model].values, var_dic['rh'][model].pressure, color=color, label=model)
        # axes[3].scatter(var_dic['q'][model].values, var_dic['q'][model].pressure, color=color, label=model)
        axes[3].plot(var_dic['q'][model].values, var_dic['q'][model].pressure, color=color, label=model)
