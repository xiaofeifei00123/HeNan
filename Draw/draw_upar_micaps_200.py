...
def get_analysis(dic={'var':'height', 'level':'500', 'time':pd.Timestamp('2021-07-20 0800') }):
    ...
    print(dic['time'].strftime('%Y-%m-%d %H%M'))
...
def add_ticks(ax,):
    """添加坐标标签"""
    ax.set_yticks(np.arange(10, 55 + 1, 10))
...
def draw_south_sea(fig,):
    pass
    ax2 = fig.add_axes([0.102, 0.157, 0.2, 0.2],projection=ccrs.PlateCarree())
...
def draw(hgt_list, tmp_list, ddf, dic):
    ...
    for df in tmp_list:
        ...
        c = ax.plot(df['lon'], df['lat'].values,transform=ccrs.PlateCarree(), color='red')
