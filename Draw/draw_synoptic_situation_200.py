...
class Map():
    ...
    def create_map(self,):
        ...
        gl.xlabel_style = {'size': 22}  #修改经纬度字体大小
...
class DataShow():
    ...
    def draw_barbas(self,u,v):
        ...
        self.ax.barbs(
            x, y, u.values, v.values,length=6,pivot='middle',
    ...
    def draw_quiver(self,u,v):
        ...
        ## 1英寸地图长度代表40m/s, u,v 的40个单位
        Q = ax.quiver(x, y, u.values,v.values,units='inches',scale=20,pivot='middle', transform=ccrs.PlateCarree())  # 绘制风矢
        # Q = ax.quiver(x, y, u.values,v.values,units='width',scale=10,pivot='middle', transform=ccrs.PlateCarree())  # 绘制风矢
        # qk = ax.quiverkey(Q, X=0.7, Y=0.15, U=40, label=r'$40\ m/s$', labelpos='E',coordinates='figure')   # 设置参考风矢
        qk = ax.quiverkey(Q, X=0.8, Y=0.18, U=10, label=r'$10\ m/s$', labelpos='E',coordinates='figure', fontproperties={'size':25})   # 设置参考风矢
...
class Figure():
    ...
    def one_subplot(self,dict):
        ...
        cs = pr.draw_picture(fig, dict, picture_dic)
        cb = fig.colorbar(
            cs,
...
class Draw():
    ...
    def draw_multi_pool(self,):
        ...
        # for pre in pre_list:
        pool = Pool(4)
        time_index = pd.date_range(start='2021-07-20 00', end='2021-07-20 12', freq='1H')
        # for t in ds.time:
        for t in time_index:
...
ds = gd.get_data()
ds
fltime = pd.Timestamp('2021-07-20 0000')
kw = {'pre':200, 'select_time':fltime}
dr = Draw()
