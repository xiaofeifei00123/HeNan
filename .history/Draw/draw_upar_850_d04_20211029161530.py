...
dds
# ds = ds.isel(time=0).sel(pressure=)
# ds.sel(pressure=[700, 500])
# %%
# ds = dds
def get_qdiv(ds):
    """计算单层水汽通量散度
    ...
    Args:
        ds (Dataset): lat, lon坐标的多维数组, 包含有q,u,v等变量
...
def caculate_q_div_integral(dds):
    """计算单个时次整层水汽通量散度

    Args:
        dds (Dataset): 包含u,v,q变量, 维度是lat, lon, pressure三维, 单个时次的
