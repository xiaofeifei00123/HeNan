...
class qv():
    ...
    def caculate_integral(self, dds):
        ...
        for i in lev.values:
            ds_one = dds.sel(pressure=i)
            qv_ds = self.caculate_qv(ds_one)  # qv, qu, qf三个变量同时处理了
            qv_da = qv_ds.to_array(dim='model')
            qv_list.append(qv_da)  # 水汽通量大小
        return qv_da
