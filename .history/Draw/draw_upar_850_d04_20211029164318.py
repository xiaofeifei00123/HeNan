...
def caculate_qv_div_integral(dds):
    """计算单个时次整层水汽通量散度
    ...
    for i in lev.values:
        ...
        # print(ds_one.pressure)
        qv_div = caculate_qv_div(ds_one)
...
def caculate_qv_div_time(ds):
    ...
    qv_div_list = []
    for t in tt:
        qvd = caculate_qv_div(ds.sel(time=t))
        qv_div_list.append(qvd)
    qv_div_dual_time = xr.concat(qv_div_list, dim='time')
    return qv_div_dual_time
        
def caculate_qv_div_integral_time(ds):
    # 计算多层，多个时次的整层水汽通量散度
    pass
    tt = ds.time
    qv_div_integral_list = []
    for t in tt:
        qvdi = caculate_qv_div_integral(ds.sel(time=t))
        qv_div_integral_list.append(qvd)
    qv_div_dual_time = xr.concat(qv_div_list, dim='time')
    return qv_div_dual_time
...
caculate_qv_div_integral_time(dds)
# da = caculate_qv_div_time(dds)

# %%
# da
# xr.concat(li, dim='time')
# da
# da
# dds = ds.sel(pressure=[1000, 925, 850, 700, 500]).isel(time=0)
# dds
