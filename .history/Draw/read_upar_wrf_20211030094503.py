...
class GetUpar():
    ...
    def get_upar_one(self, fl):
        ...
        pre_level = [1000, 975, 950, 925, 900, 850, 800, 700, 600, 500,400, 300, 200, 100, 60]
...
def combine_one():
    ...
    gu = GetUpar()
    # for t in time_list:
    path_wrfout = path_main+'HighResolution'
    ds = gu.get_upar(path_wrfout)
    # flnm = 'high_resolution_high_hgt_upar_d04.nc'
    flnm = 'high_resolution_upar_d04.nc'
...
def regrid_one():
    ...
    ds = xr.open_dataset(path_in)
    ds_out = regrid_xesmf(ds, area)
    path_out = path_main+'high_resolution_upar_d04_latlon.nc'
    # path_out = path_main+'high_resolution_high_hgt_upar_d04_latlon.nc'
    ds_out = ds_out.rename({'ua':'u', 'va':'v', 'geopt':'height'})
    ds_out.to_netcdf(path_out)
