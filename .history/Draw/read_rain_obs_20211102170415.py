...
da
dain = da.sel(time=slice('2021-07-20 00', '2021-07-20 12')).sum(dim='time')
dain

from pykrige.ok import OrdinaryKriging
krige = OrdinaryKriging(da.lon.values,da.lat.values,da_hour.values,variogram_model="gaussian",nlags=6, verbose=False,enable_plotting=False)
# krige = OrdinaryKriging(da.lon.values,da.lat.values,da.values,variogram_model="gaussian",nlags=6)
# print("插值完成一次")
hour_grid,ss = krige.execute("grid", lon_grid, lat_grid)
