...
def get_rain_model(path_dic):
    pass
    ds = get_rain(path_dic['path_main'])
    print(ds.max())
...
def regrid_rain_model(path_dic):
    ds = regrid(path_dic['path_rain'])
    ds.to_netcdf(path_dic['path_rain_latlon'])
...
def main():
    ...
    get_rain_model(path_dic)
    regrid_rain_model(path_dic)
    pass

def dual():
    pass
    model_list = ['1900_90m', '1912_900m', '1912']
    path_main = '/mnt/zfm_18T/fengxiang/HeNan/Data/1900_90m/'
    path_dic = {
        'path_main':path_main,  # 模式数据文件夹
        'path_rain':path_main+'rain.nc', # 原始降水存储路径+文件名
