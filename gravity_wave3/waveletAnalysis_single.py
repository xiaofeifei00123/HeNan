...
def get_data_rain_station():

    flnm_obs = '/mnt/zfm_18T/fengxiang/HeNan/Data/OBS/rain.nc'
    ds_obs = xr.open_dataarray(flnm_obs)
    da = ds_obs.sel(id='57083')
    # da.plot()
    sst = da.values
    tt = da.time.values
    return sst, tt, da
...
def get_data_vertical_speed():
    ...
    # flnm = '/mnt/zfm_18T/fengxiang/HeNan/Data/GWD/d03/newall/CTRL/wrfout/time_cross_F.nc'
...
sst, time, da = get_data_rain_station()
# %%
cm = 1/2.54
fig = plt.figure(figsize=(8*cm, 4*cm), dpi=600)
ax = fig.add_axes([0.15, 0.2, 0.7, 0.7])
x = np.arange(len(time))
ax.bar(x, sst, width=0.8)
ax.set_xlim(20, 40)
# ax.plot(time, sst)
# da.plot()
# %%
...
# fig.savefig(figpath+'wave_time_4km_ctrl')
# fig.savefig(figpath+'wave_time_4km_gwd3')
# fig.savefig(figpath+'wave_rain')
fig.savefig(figpath+'wave_rain_station')
