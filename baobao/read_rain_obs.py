...
class RainStation():
    ...
    def get_rain_station(self,):
        ...
        path='/mnt/zfm_18T/fengxiang/HeNan/baobao'
        fl_list = os.popen('ls {}/1708*.000'.format(path))  # 打开一个管道
    ...
    def save_rain_station(self,):
        ...
        rain_st.to_netcdf('/mnt/zfm_18T/fengxiang/HeNan/baobao/rain_station.nc')
...
class RainGrid():
    ...
    def rain_station2grid(self, da):
        ...
        lon_grid = np.arange(116-1, 123+1, 0.125)
        lat_grid = np.arange(31-1, 35+1, 0.125)
    ...
    def save_rain_grid(self,):
        ## 读取存储好的站点数据
        da = xr.open_dataarray('/mnt/zfm_18T/fengxiang/HeNan/baobao/rain_station.nc')
        ## 插值为格点数据
        da_grid = self.rain_station2grid(da)
        da_grid.to_netcdf('/mnt/zfm_18T/fengxiang/HeNan/baobao/rain_grid.nc')
